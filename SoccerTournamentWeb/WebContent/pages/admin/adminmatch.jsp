...
<body>
	...
	<div id="body">
		...
		<c:if test="<%=rencontre.getFin() == null%>">
			<a
				href="<%=Controleur.SERVLET_PATH + Controleur.ACTION_TOURNAMENT %>&id=<%=rencontre.getNomTournoi()%>"><%=rencontre.getNomTournoi()%></a>,
			tour ${rencontre.tour}<br /> Hôtes: <a
				href="<%=Controleur.SERVLET_PATH + Controleur.ACTION_TEAM %>&id=<%=rencontre.getHotes().getNom()%>"><%=rencontre.getHotes().getNom()%></a><br />
			Visiteurs: <a
			...
			<c:if test="<%=!nonValidable%>">
				<form method="post" action="<%=Controleur.SERVLET_PATH + Controleur.ACTION_VALIDATE_MATCH%>&id=<%=request.getParameter("id")%>">
					<fieldset>
